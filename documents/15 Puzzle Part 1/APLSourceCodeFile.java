...
// Code Submission for Artificial Intelligence Assignment 1
// Andrew Lenz
...
class SearchManager {
    ...
    HashMap<String, State> VisitedBoards;
    boolean endFound;
    ...
    public void PerformBreadthFirstSearch() {
        ...
        while (!endFound) {
            ...
            VisitedBoards.put(examinedState.Board.toString(), examinedState);
            ...
            for (Swap s:possibleSwaps) {
                ...
                if (!VisitedBoards.containsKey(tempBoard.toString())) {
                    ...
                    /* Create the next State
                        Properties of next State:
                            ...
                            Hn is 0 because this is not a heuristic search
                            Priority is examined state's priority + 1 level lower
                    */
                    State nextState = new State(examinedState.StateID, tempBoard, examinedState.Gn + cost, 0, examinedState.Priority + 1);
        ...
        // Print Everything
        PrintResults("Breadth First");
    }

    // Performs Heuristic 1 or 2 search given the passed interval
    public void PerformHeuristicSearch(int h) {
        if (ResetSearch())
        ...
        while (!endFound) {
            ...
            for (Swap s:possibleSwaps) {
                ...
                if (!VisitedBoards.containsKey(tempBoard.toString())) {
                    ...
                    int hn;
                    if (h == 1)
                        hn = Heuristic1(tempBoard);
                    else
                        hn = Heuristic2(tempBoard);
                    /* Create the next State
                        Properties of next State:
                            ParentID is the StateID of the currently examined State
                            Board is the tempBoard generated from the swap above
        ...
        if (h == 1)
            PrintResults("A* Heuristic 1");
        else
            PrintResults("A* Heuristic 2");
    }

    // Compares a board to the EndBoard to determine an H(n) value for the board using heuristic 1
    public int Heuristic1(int[] board) {
        int estimatedCost = 0;

        // Iterates over the entire board to count the number of tiles that do not match the EndBoard
        for (int i = 0; i < board.length; i++) {
            if (board[i] != EndBoard[i])
                // If the value at i in the board does not match the EndBoard, increment the cost
                if (board[i] >= 10)
                    // cost incremented by 10 because these tiles are more expensive to move
                    estimatedCost += 10;
                else
                    estimatedCost++;
        }
    ...
    public int Heuristic2(int[] board) {
        int estimatedCost = 0;
        ...
        for (int val = 0; val < board.length; val++) {
            ...
            if (val >= 10)
                // Multiply distance by 10 to reflect the cost of moving the more expensive piece
                distance *= 10;
...
public class APLSourceCodeFile {
    public static void main(String[] args){
        ...
        manager.PerformBreadthFirstSearch();
        manager.PerformHeuristicSearch(1);
        manager.PerformHeuristicSearch(2);
    }
}
